...
(function( $ ) {
	...
	var ed, node, hasI, iconStackWrapperNode, tdIconLists, tdTitle, iconSizeLabel, iconAlignLabel, tdHoverColorWrapper,
		tdInputs = {},
		autocompleteData = [],
		...
		requiredClassesArr = [ 'iconized', 'mceItemNoResize','iconized-stack-base' ], reservedClasses = ['iconized-stack'],
	...
	iconizeTinymceDialog = {
		...
		init: function() {
			...
			if ( typeof( fullscreen ) !== 'undefined' ) {
				...
				fullscreen.insertIcon = function() {
		...
		open: function() {
			...
			iconizeTinymceDialog.textarea = $('#'+wpActiveEditor).get(0);
		...
		mceRefresh: function() {
			...
			if ( hasI ) { // Node is iconized
				...
				//var parents = ed.dom.getParents( node, '*', ed.getBody() );
				//for ( i = 0; i<parents.length; i++ ) { // to do }
		...
		mceUpdate: function( ed ) {
			...
			if ( ! iconname ) {
				...
				if ( hasI ) {
					...
					classes  = 'iconized mceItemNoResize '+iconset+' glyph-'+ iconname;
					...
					icon = ed.dom.create(
						'span',
						{ 'class': classes, 'contenteditable': 'false'},
						'<span class="iconized-placeholder mceItemHidden mceItemNoResize">|icon|</span>'
					);
